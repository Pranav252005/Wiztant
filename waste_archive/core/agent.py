...
def _get_user_id() -> str:
    """Return a stable user id for Tune Hub."""
    try:
...
_CONVERSATION_HISTORY_PATH = Path("data") / "conversation_history.json"


def _save_conversation_history() -> None:
    """Persist conversation history to disk so it survives restarts."""
    try:
        _CONVERSATION_HISTORY_PATH.parent.mkdir(parents=True, exist_ok=True)
        tmp = _CONVERSATION_HISTORY_PATH.with_suffix(".tmp")
        ...
        tmp.replace(_CONVERSATION_HISTORY_PATH)
    except Exception as e:
...
def _load_conversation_history() -> list:
    ...
    if not _CONVERSATION_HISTORY_PATH.exists():
        return []
    try:
        with open(_CONVERSATION_HISTORY_PATH, "r", encoding="utf-8") as f:
            data = json.load(f)
            msgs = data.get("messages", [])
            # Sanity check: must be a list of dicts with role/content
            if isinstance(msgs, list) and all(
                isinstance(m, dict) and "role" in m and "content" in m for m in msgs
            ):
                return msgs
    except Exception as e:
        print(f"[Agent] History load error: {e}")
    return []


# Load persisted conversation history on import so it survives restarts.
_persisted_history = _load_conversation_history()
if _persisted_history:
    state.conversation_history = _persisted_history
    conversation_history = state.conversation_history
    print(f"[Agent] Restored {len(_persisted_history)} conversation messages from disk")
...
def _notify_overlay_history_updated():
    ...
    if overlay:
        try:
            ...
            pass
    # Also push to React overlay via WS bridge
    try:
        from core.ws_bridge import send_history_update
        send_history_update()
    except Exception:
        pass
...
def ask_ai(user_text: str, user_already_added: bool = False, force_agent: bool = False):
    ...
    if state.agent_mode or force_agent:
        ...
        # Tune Hub: apply learned agent tuning if available
        tuned_task = user_text
        try:
            ...
            if middleware:
                tuned = middleware.apply(
                    ...
                    feature_name="agent",
                    task=user_text,
                    feature_input={"task": user_text},
                )
                tuned_task = tuned.get("task", user_text)
                if tuned_task != user_text:
                    print(f"[TuneHub] Agent tune applied: {user_text[:60]}... → {tuned_task[:60]}...")
        except Exception:
            pass
