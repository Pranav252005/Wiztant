...
def validate_coordinates(x: int, y: int, screen_w: int = 1920, screen_h: int = 1080) -> tuple[bool, str]:
    ...
    if x < SCREEN_MIN_X or y < SCREEN_MIN_Y:
        return False, f"coord_too_low:({x},{y})"
    if x > screen_w - SCREEN_MIN_X or y > screen_h - SCREEN_MIN_Y:
        return False, f"coord_out_of_bounds:({x},{y}) screen=({screen_w},{screen_h})"
    return True, ""
