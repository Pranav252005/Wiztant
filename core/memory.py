...
def _ensure_loaded() -> None:
    """Reload from disk if _data was reset (e.g. module reloaded during dev)."""
    global _data
...
Valid section keys: identity, preferences, current_projects, tools_and_tech,
                    goals, context, notes, agent_tasks
    ...
    def _extract():
        try:
            ...
            valid_sections = {
                ...
                "tools_and_tech", "goals", "context", "notes", "agent_tasks"
            }
            changed = False
            with _lock:
...
def store_agent_task(task_text: str, result: str) -> None:
    """Store an agent task and its outcome so the AI remembers what it has done."""
    _ensure_loaded()
    if not _data.get("memory_enabled", True):
        return
    key = task_text.strip()
    ...
    if not key or not value:
        return
    with _lock:
        _data.setdefault("agent_tasks", {})[key] = value
        _save(_data)
    print(f"[Memory] Agent task stored: {key[:60]}...")


def update_from_tune(category: str, key: str, value: str) -> bool:
    """Directly update a memory entry from the Tune hub. Returns True if saved."""
    global _data
    _ensure_loaded()
    valid_sections = {
        "identity", "preferences", "current_projects",
    ...
    with _lock:
        if category not in _data:
            _data[category] = {}
        _data[category][str(key)] = str(value).strip()
        _save(_data)
    print(f"[Memory] Tune update: {category}.{key} = {value}")
    return True
